      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-200 pb-8">
        ...
        <div className="bg-white border border-slate-200 p-4 rounded-2xl flex items-center shadow-sm">
          <div className="text-center px-4 border-r border-slate-100">
            ...
            <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mt-1">Avg Rating</p>
          </div>
          <div className="px-4">
             <div className="flex space-x-0.5">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star 
                    key={star} 
                    className={`w-3 h-3 ${star <= Math.round(summary.averageRating) ? 'fill-blue-600 text-blue-600' : 'text-slate-200'}`} 
                  />
                ))}
             </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    >
                      <Star 
                        className={`w-8 h-8 transition-colors duration-200 ${
                          star <= (hover || rating) ? 'fill-blue-600 text-blue-600' : 'text-slate-100'
                        }`} 
                      />
                    </button>
                  ))}
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full rounded-2xl border border-slate-200 p-4 min-h-[120px] focus:ring-4 focus:ring-blue-500/5 focus:border-blue-500 outline-none transition font-medium text-slate-900"
                  required
                />
              </div>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-blue-200 hover:bg-blue-700 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center space-x-2 disabled:opacity-50"
              >
                {submitting ? (
              ...
              {!isAuthenticated() && (
                <p className="text-[10px] text-center text-slate-400 font-bold uppercase tracking-wider">
                  Post anonymously or <Link to="/login" className="text-blue-600 hover:underline">sign in</Link>
                </p>
              )}
            </form>
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          {loading ? (
            ...
            reviews.map((review) => (
              <div key={review._id} className="bg-white rounded-[2rem] border border-slate-100 p-8 shadow-sm hover:shadow-xl hover:shadow-blue-900/5 transition-all duration-500">
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
                  ...
                  <div className="flex space-x-1 bg-slate-50 px-3 py-1.5 rounded-full border border-slate-100">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star 
                        key={star} 
                        className={`w-3 h-3 ${star <= review.rating ? 'fill-blue-600 text-blue-600' : 'text-slate-200'}`} 
                      />
                    ))}
                  </div>
                </div>
