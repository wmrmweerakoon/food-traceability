        ...
        <div className="relative z-10">
          <Link to="/" className="inline-flex items-center space-x-3 mb-12 group">
            <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-2xl group-hover:scale-110 transition-transform duration-500">
              <LogIn className="w-6 h-6 text-green-900" />
            </div>
        ...
        <div className="max-w-md w-full relative z-10">
          <div className="bg-white lg:bg-transparent rounded-[2.5rem] p-10 lg:p-0 shadow-2xl shadow-slate-200 lg:shadow-none border border-slate-100 lg:border-none">
            <div className="mb-10 lg:text-left text-center">
              <div className="lg:hidden inline-flex items-center justify-center w-16 h-16 bg-green-50 rounded-2xl mb-4 border border-green-100">
                <LogIn className="w-8 h-8 text-green-700" />
              </div>
              <h1 className="text-4xl font-black text-green-950 tracking-tight mb-2">Welcome Back.</h1>
              <p className="text-slate-600 font-bold">Sign in to your account</p>
            </div>

            {error && (
              <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-2xl text-red-700 text-sm font-bold animate-shake">
                {error}
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="pt-2">
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-green-900 text-white py-5 rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-green-900/20 hover:bg-green-800 hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 disabled:opacity-50"
                >
            ...
            <div className="mt-10 lg:text-left text-center">
              <p className="text-slate-600 font-bold uppercase tracking-widest text-[11px]">
                New to AgriTrace?{' '}
                <Link to="/register" className="text-green-700 hover:text-green-800 transition-colors ml-1 font-black underline underline-offset-4 decoration-2">
                  Sign Up
                </Link>
              </p>
            </div>
