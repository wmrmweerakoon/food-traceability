...
const deleteInventoryItem = async(req, res) => {
    try {
        ...
        await StoreInventory.findByIdAndDelete(id);
        res.status(200).json({
            success: true,
...
const addProductToInventory = async(req, res) => {
    try {
        ...
        const newInventoryItem = new StoreInventory({
            ...
            // Add required top-level fields for StoreInventory schema
            batchId: productBatch.batchId, // string batchId
            storeId: req.body.storeId, // from request body
            expiryDate: req.body.manualExpiry || productBatch.expiryDate, // use manualExpiry if provided, else batch expiry
